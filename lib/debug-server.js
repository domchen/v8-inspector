...
function handleWebSocketConnection(socket) {
    ...
    var debugHost = this._getDebuggerHost(socket.upgradeReq.url);
    this._createSession(debugPort, debugHost, socket);
}
...
DebugServer.prototype.start = function (options) {
    ...
    app.get('/log', logAction.bind(this));
...
DebugServer.prototype._getUrlFromReq = function (req) {
    ...
    return buildInspectorUrl(urlParts[0], urlParts[1], debugPort, debugHost, this._isHTTPS);
};

DebugServer.prototype._createSession = function (debugPort, debugHost, wsConnection) {
    return new Session(this._config, debugPort, debugHost, wsConnection);
};
...
DebugServer.prototype.address = function () {
    ...
    var args = [config.webHost, address.port, config.debugPort, config.debugHost, this._isHTTPS];
